...
figuring out what data you're looking at 
-   What is the purpose of writing `x = rating`?
to set x to a variable
...
```{r}
# It added bars with the + geom_bar()
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```

```{r}
# This is just labels for the X and Y axis
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```{r}
# This made all of the bars blue using a fill argument
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
...
```{r}
# This made it Orange and Blue by giving the graph a color argument and a fill argument
ggplot(hikes, aes(x = rating)) +
...
```{r}
# It now is using a theme called minimal theme with the same arguments as before with the same data
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
...
#there are too many mountains you cant visualize the data at all.
...
```{r}
# there is an outline and there is a filled in color
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white", fill = "blue") 
```

```{r}
# they put the labels on the x and y axis
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") +
...
```{r}
# they made the bins really small 
ggplot(hikes, aes(x = elevation)) +
...
geom_histogram()
-   What's the difference between `color` and `fill`?
Color is the outline and fill is like the fill tool in ms paint
-   Why does adding `color = "white"` improve the visualization?
because it makes a clear border between the data bins from eachother and the background
-   What did `binwidth` do?
it changes how big the bins are so like they go into chunks counting from that idk how to explain it but yk ik right
-   Why does the histogram become ineffective if the `binwidth` is too big (eg, 1000 feet)?
because everything falls on one side or the other 
-   Why does the histogram become ineffective if the `binwidth` is too small (eg, 5 feet)?
there are too many and a difference of five feet is insignificant in this case.
...
-   INTUITION CHECK: Before tweaking the code and thinking back to `geom_bar()` and `geom_histogram()`, how do you *anticipate* the following code will change the plot?
    ...
    the color will be the line
    -   `geom_density(fill = "orange")`
    fill will fill the bottom of the graph 
...
# I AM A FREAKING GENIUS I KNEW IT!!!
...
because it makes your code look clean and understandable to anybody reading it including yourself 
