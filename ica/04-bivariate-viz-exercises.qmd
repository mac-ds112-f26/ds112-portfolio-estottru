...
How does the data split up along geographical lines.
...
ggplot(elections, aes(x = repub_pct_20)) +
  geom_density(color = "white", fill = "orange")
```
...
counties in Texas. They go from a smaller percentage in 2016 to jumping in 2020
...
```{r}
#median rent is a far better predictor based on both what i can see visually and the print(cor) thing I did under this. Because median rent and elections has a value closer to 1/-1 there is a stronger correlation between the two variables. 
print(cor(elections$median_rent, elections$repub_pct_20))

print(cor(elections$median_age, elections$repub_pct_20))

# Scatterplot of repub_pct_20 vs median_rent
# summarize(cor) <- remember this! 
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
#its visually hard to interpret and its hard to see what percent is in which catagory or what state they're from.
#you can see a slight difference if a county is nearly 100 percent red then historically it is probably that way too?
#there is a categorical variable in the x axis which is NO GOOD
```
...
there are many outliers in the historically red counties that lost a majority of the repub support. It also seems like support by percentage in the counties skews red.
...
```{r}
# Name two "bad" things about this plot, its not transparent I cant see the full shape of the historical data. I also think that the colors should match up with what is being said. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r}
# What does scale_fill_manual do? it fixes the colors so they line up
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r}
# What does facet_wrap do?! It splits it up into three separate graphs. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# Everything is on top of each other and its nonsense and unreadable for data 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
ANSWER: I LIKE THE DENSITY PLOT WHEN ITS TRANSPARENT 

-   What is one pro of density plots relative to boxplots?

ANSWER: THE DENSITY PLOT DISTRIBUTES THE DATA MORE NATURALLY NOT JUST SPLITTING IT UP INTO FOUR CATAGORIES AND OUTLIERS 

-   What is one con of density plots relative to boxplots?

ANSWER: DENSITY PLOTS DONT GIVE YOU THE MEDIAN 
...
#ITS TAKING ALL OF THE HISTORICAL BLUE, PURPLE, & RED COUNTIES AND SPLITTING THEM UP BASED ON WHO WON THEM IN 2020
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
...
# dodge argument places them side by side 
ggplot(elections, aes(x = historical, fill = winner_20)) +
...
# this is based on percentage and keeps them stacked on each other it doesn't take the scale into account.
...
PROPORTIONAL LOOKS AT PERCENTAGES AND MAKES COMPARISON EASY 

#### Part b

What's your favorite bar plot from part and why?

I REALLY LIKE THE PROPORTIONAL BAR PLOT BECAUSE I CAN COMPARE PERCENTAGES ACROSS DATA SCALES 
